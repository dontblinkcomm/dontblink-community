import{am as l}from"./index-Ch7xXdd3.js";const a="https://dontblink-thesis.lawson-e69.workers.dev",d=t=>`dontblink thesis
stock:${t.stock}
ticker:${t.ticker}
name:${t.name}
...
amount:${s}
purpose:${o}
payto:${n.toLowerCase()}`;async function L(t){const s=await fetch(`${a}/grants?thesis=${t}`);if(!s.ok)throw new Error(`grants ${s.status}`);return(await s.json()).grants??[]}async function q(t,s,o,n,e){const r=await i(t,b(s,o,n,e)),c=await fetch(`${a}/grant`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({id:s,requester:t,amount:o,purpose:n,payto:e,sig:r})}),f=await c.json();if(!c.ok||!f.ok)throw new Error(f.error??`grant ${c.status}`)}export{v as a,J as b,S as c,N as d,L as e,x as f,P as g,C as h,j as i,E as j,T as p,q as r,M as s,O as v};
