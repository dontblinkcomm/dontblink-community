# GMGN 平台来源 Logo 打标交接包

**本次交付重点（2026-09-17）：平台来源 Logo 角标与新币同步，和多链 Launch 分开验收。最简接入入口、固定品牌图档和逐币查询规则见 [GMGN-PLATFORM-BADGE.md](GMGN-PLATFORM-BADGE.md)。** 来源查询已有实现；新增角标字段与品牌资源随正常 source CI 发布，实际可用性以对应发布证据及 HTTP 验收为准。GMGN 接收合同和实际角标显示尚未确认。
本包不代表 GMGN 已接受合作、已提交项目、已收录币种，也不会主动向 GMGN 发送任何资料。

## 交付范围：平台来源 Logo 打标

用户要的是 GMGN 代币头像旁的 **dontblink 发射平台角标**。每次 dontblink 发射成功，自动向 GMGN 同步“链 + 代币地址 → dontblink 来源及 Logo”，供其产品识别并显示。不是调用 API 帮用户发币，也不是以项目生命周期展示为目的。

| 独立工作 | 完成条件 | 当前边界 |
|---|---|---|
| GMGN 来源打标与新币通知 | dontblink 平台名称/Logo 由 GMGN 注册；已有币全量回补；每个确认发射的新币自动同步；可查重、重试、补发并核实 GMGN 实际显示角标 | 已有来源查询 API 主要覆盖 RH；没有已确认的 GMGN 通知接收端点、鉴权及回执合同。仅发布 JSON 不能称“已通知 GMGN” |
| 多链 Launch | 各链在统一页面完成钱包、配置、预算、模拟、签名、回执与结果流程，并完成真实链上验收 | Solana、Base、Morph 仍待各自部署和验收；来源 API 有链字段不等于该链能发射 |

GMGN 打标不用等待新链全部上线。对当前实际发射来源先回补和同步；新链正式启用时增加同一来源合同的 adapter。
完整性必须覆盖已确认的全部 dontblink 发射，不能只取首页、已有报价或已毕业建池的币。

### 通知合同待双方确认

1. **品牌**：固定平台 ID `dontblink`、展示名 `dontblink`、平台 Logo（与每个币自己的 token logo 区分）。GMGN 的内部平台 ID 和图标映射由其确认，不能假设我们发出字段就会自动显示。
2. **识别**：`chain + token` 是稳定身份；附发射交易/事件、部署器/程序、确认区块或 slot 及观察时间。共有底层工厂本身不是 dontblink 来源证明。
3. **触发**：后端确认发射成功后记录新币，失败/拒签/仅有草稿不发通知。页面关闭不能导致漏报；历史事件扫描负责补发。
4. **方式**：优先双方约定的 webhook 推送；若 GMGN 要定时拉取，则提供完整快照和可恢复的增量读取。其接收 URL、鉴权、频率、确认深度、回执语义目前未给定，不猜测接口。
5. **可靠性**：固定事件 ID、持久化发件队列、幂等键、退避重试、死信和人工重放；补扫及链重组修正。通知失败不能导致已成功的用户发射被显示为失败，也不能重复发射。
6. **验收**：模拟通知测试与真实 GMGN 回执分开；HTTP 收到通知与 GMGN 界面已打标分开。至少以一个已确认的新币核对真实角标后，才宣告联调成功。

GMGN 公开查询资料把 `logo`（币的图片）、`launchpad` 与 `launchpad_platform` 分开定义；这是查询数据字段，不是公开打标写入合同。参考 [GMGN 官方 token 字段说明](https://github.com/GMGNAI/gmgn-skills/blob/main/skills/gmgn-token/SKILL.md)。

下面的 lifecycle 资料为已有可选扩展，不是这次来源打标的交付依赖。
