 ...
 * **还没有值的排最前面,而不是只按时间。**
 *
 * 原来只按 `at` 从旧到新。问题是待重试的空 K 线池子被我写了 `at = now - 6h`
 * (想着「过六小时再查」),而成功刷新的写的是 `at = now` —— 结果队列优先挑的是
 * **更早刷过的、已经有值的**池子,那批没值的永远排不到前面。
 *
 * 实测:连着三轮「本轮刷新 25/220」而覆盖 +0 —— 25 个全是已覆盖的在重刷,
 * 剩下的 244 个一个都没轮到。**「在干活」和「有进展」是两回事**,
 * 而只看刷新数会把前者当成后者。
 *
 * 排序改成:没值的优先 → 其次最久没刷的 → 再次 24h 成交量大的。
 * 覆盖率是我们在追的东西,就该让它排第一。
 */
const hasVal = (p) => typeof store[p]?.v === 'number'
const queue = pools
  .map((p) => ({ p, at: store[p]?.at ?? 0, v: vol24.get(p) ?? 0, need: hasVal(p) ? 1 : 0 }))
  .sort((a, b) => (a.need - b.need) || (a.at - b.at) || (b.v - a.v))
  .slice(0, BATCH)
