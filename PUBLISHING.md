...
**基名从产物里消失有两种可能，只有一种是事故：**

| 情况 | 判据 | 该怎么办 |
|---|---|---|
| **rollup 把它并进了别的 chunk** | 源码树里**还找得到**这个模块 | 放行，注明「这次被并进别的 chunk」 |
| **真的是另一棵树** | 源码树里**也找不到** | 停，点名它 |

不加这一步，检查会天天误报。实测踩过：`queue` 报丢失，查了 `web/src/lib/queue.ts`
好端端在那儿，只是这次被并进别的 chunk，功能一点没少。

**天天喊狼来了的检查比没有更糟** —— 它会被关掉，而那时所有人还以为它在守着。

```bash
for name in $(comm -23 /tmp/live-names.txt /tmp/new-names.txt); do
  # 这个模块在源码树里还在吗？在 = 只是分包变了；不在 = 真删了
  if find "$WEB/src" -iname "$name.*" | grep -q .; then
    echo "ℹ️  $name 这次被并进别的 chunk（源码还在，功能没少）"
  else
    echo "❌ $name 源码树里也找不到 —— 这是另一棵树，别发"
    FAIL=1
  fi
done
[ -n "$FAIL" ] && exit 1
...
### 还要问第三句：我要发的这个 commit，在远端吗

```bash
git ls-remote origin | grep -q "$(git rev-parse HEAD)" \
  || echo "⚠️ 这个 commit 不在远端 —— 发出去以后别人从 origin 构建会覆盖掉它。先推。"
```

**这条是给「我发的东西别人拿不到」用的。** 08-19 两条分支互相覆盖了四次，
根子就是有一边的产物领先远端，而没有任何环节会说出来。

**输出非空且源码树里也找不到，就停下。** 每一行都是一个你即将从线上删掉的页面/功能。
